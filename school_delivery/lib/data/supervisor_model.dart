...
class Supervisor {
  ...
  factory Supervisor.fromSnapshot(DocumentSnapshot<Map<String, dynamic>> document) {
...
// Fetch All uer Or User Details
  static Future<Supervisor> getSupervisorDetails(String name) async {
    final snapshot =
    await _db.collection('Supervisor').where('name', isEqualTo: name).get();
    final userData = snapshot.docs.map((e) => Supervisor.fromSnapshot(e)).single;
    return userData;
  }

  static Future<List<Supervisor>> getAllSupervisor() async {
    final snapshot = await _db.collection('Supervisor').get();
    final userData = snapshot.docs.map((e) => Supervisor.fromSnapshot(e)).toList();
    return userData;
  }
}
